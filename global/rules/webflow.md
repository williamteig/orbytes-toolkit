...
**Gotcha — when MIGRATING off Webflow, the source of truth is the rendered browser DOM, not the Designer or Figma.**
Webflow pages render client-side — the raw HTML response, the Designer UI, and the Designer API all reflect **authoring structure** (component slots, CMS bindings, conditional visibility rules) rather than what the visitor sees. The Figma file (if one exists) reflects **design intent**, not what actually shipped. For any migration off Webflow (e.g. Webflow → Astro), capture each page's rendered DOM via Chrome MCP (`mcp__Claude_in_Chrome__navigate` → `get_page_text` / `read_page`) or Claude Preview (`mcp__Claude_Preview__preview_snapshot`), save under `discovery/current-site/pages/<slug>/rendered.html`, and work from there. This is documented end-to-end in `orbytes-discovery` Step 1b. Do not rebuild copy or layout from the Designer or Figma alone — they drift from production.
